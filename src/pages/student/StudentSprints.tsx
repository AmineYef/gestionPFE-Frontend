...
import { SprintStatusBadge, PriorityBadge, ProgressBar } from "@/components/shared/Badges";
...
{mockSprints.length} sprint(s) — {mockUserStories.length} user stories
</p>
</div>
<Button size="sm">
...
const stories = mockUserStories.filter((us) => us.sprintId === sprint.id);
...
return (
<div key={sprint.id} className="rounded-xl border bg-card overflow-hidden">
{/* Sprint header */}
<button
...
{stories.map((us) => {
const usTasks = mockTasks.filter((t) => t.userStoryId === us.id);
const usDone = usTasks.filter((t) => t.status === "Done").length;
return (
<div
key={us.id}
