...
import { ValidationBadge } from "@/components/shared/Badges";
import { CheckCircle2, XCircle, Clock, User, Calendar, Link2 } from "lucide-react";
...
{pending.length} en attente · {approved.length} approuvée(s) · {rejected.length} rejetée(s)
</p>
</div>
...
{pending.length > 0 && <Section title={`⏳ En attente (${pending.length})`} items={pending} />}
{rejected.length > 0 && <Section title={`✗ Rejetées (${rejected.length})`} items={rejected} />}
{approved.length > 0 && <Section title={`✓ Approuvées (${approved.length})`} items={approved} />}
</div>
);
}
